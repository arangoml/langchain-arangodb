...
class TestArangoGraph:
    ...
    def test_get_structured_schema_returns_correct_schema(self, mock_arangodb_driver: MagicMock):
        ...
        graph._ArangoGraph__schema = test_schema  # Accessing name-mangled private attribute
    ...
    def test_arangograph_init_with_empty_credentials(self, mock_arangodb_driver: MagicMock) -> None:
    ...
    def test_arangograph_init_refresh_schema_other_err(self, mock_generate_schema, mock_arangodb_driver):
    ...
    @patch.object(ArangoGraph, "generate_schema")
    def test_refresh_schema_handles_arango_server_error(self, mock_generate_schema, mock_arangodb_driver: MagicMock):
    ...
    def test_add_graph_docs_invalid_capitalization_strategy(self, mock_arangodb_driver: MagicMock):
    ...
    def test_process_edge_as_type_full_flow(self):
        ...
        assert edge_defs["sanitized_LIKES"]["from_vertex_collections"] == {"sanitized_User"}
        assert edge_defs["sanitized_LIKES"]["to_vertex_collections"] == {"sanitized_Item"}
    ...
    def test_add_graph_documents_full_flow(self, graph):
        ...
        graph_doc = GraphDocument(nodes=[node1, node2], relationships=[edge], source=source_doc)
    ...
    def test_get_node_key_handles_existing_and_new_node(self):
        ...
        process_node_fn.assert_called_once_with(expected_key, new_node, nodes, entity_collection_name)
    ...
    def test_hash_raises_on_unstringable_input(self):
        ...
        with pytest.raises(ValueError, match="Value must be a string or have a string representation"):
            self.graph._hash(BadStr())

    def test_hash_uses_farmhash(self):
        with patch("langchain_arangodb.graphs.arangodb_graph.farmhash.Fingerprint64") as mock_farmhash:
    ...
    def test_sanitize_input_string_above_limit(self, graph):
        result = graph._sanitize_input({"text": "a" * 50}, list_limit=5, string_limit=10)
        assert result == {"text": "String of 50 characters"}


    def test_sanitize_input_small_list(self, graph):
        result = graph._sanitize_input({"data": [1, 2, 3]}, list_limit=5, string_limit=10)
        assert result == {"data": [1, 2, 3]}


    def test_sanitize_input_large_list(self, graph):
        result = graph._sanitize_input({"data": [0] * 10}, list_limit=5, string_limit=10)
    ...
    def test_sanitize_input_nested_dict(self, graph):
        ...
        assert result == {"level1": {"level2": {"long_string": "String of 100 characters"}}}
    ...
    def test_from_db_credentials_uses_env_vars(self, monkeypatch):
        ...
        with patch.object(get_arangodb_client.__globals__['ArangoClient'], 'db') as mock_db:
            fake_db = MagicMock()
    ...
    def test_generate_schema_invalid_sample_ratio(self):
        with pytest.raises(ValueError, match=r"\*\*sample_ratio\*\* value must be in between 0 to 1"):
    ...
    def test_refresh_schema_sets_internal_schema(self):
        ...
        self.graph.refresh_schema(sample_ratio=0.5, graph_name="TestGraph", include_examples=False, list_limit=10)
    ...
    def test_add_graph_documents_capitalization_strategy(self, strategy, input_id, expected_id):
        graph = ArangoGraph(db=MagicMock(), generate_schema_on_init=False)
