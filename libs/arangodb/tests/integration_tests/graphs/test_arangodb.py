...
def test_schema_refresh_updates_schema(db: StandardDatabase):
    ...
    assert any(col["name"].lower() == "entity" for col in graph.schema["collection_schema"])
...
def test_sanitize_collection_name(db: StandardDatabase):
    ...
    assert graph._sanitize_collection_name("1invalidStart") == "Collection_1invalidStart"
...
def test_process_edge_as_type(db):
    ...
    assert sanitized_source_type in edge_definitions_dict[sanitized_edge_type]["from_vertex_collections"]
    assert sanitized_target_type in edge_definitions_dict[sanitized_edge_type]["to_vertex_collections"]
...
def test_graph_creation_and_edge_definitions(db: StandardDatabase):
    ...
    member_def = next(e for e in edge_definitions if e["edge_collection"] == "MEMBER_OF")
...
def test_graph_edge_definition_replacement(db: StandardDatabase):
    ...
    g = db.graph(graph_name)
    edge_defs_1 = [ed for ed in g.edge_definitions() if ed["edge_collection"] == "CONNECTS"]
    ...
    edge_defs_2 = [ed for ed in g.edge_definitions() if ed["edge_collection"] == "CONNECTS"]
...
def test_capitalization_strategy_applied(db: StandardDatabase, strategy: str, expected_id: str):
    graph = ArangoGraph(db, generate_schema_on_init=False)
...
def test_create_edge_definition_called_when_missing(db: StandardDatabase):
    ...
    assert mock_graph.create_edge_definition.called, "Expected create_edge_definition to be called"
...
def test_embed_relationships_and_include_source(db):
    ...
    assert any("embedding" in e for e in all_relationship_edges), "Expected embedding in relationship"
    assert any("source_id" in e for e in all_relationship_edges), "Expected source_id in relationship"
