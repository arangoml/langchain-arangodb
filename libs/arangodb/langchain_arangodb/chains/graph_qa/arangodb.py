    ...
    *Security note*: Make sure that the database connection uses credentials
    ...
    def from_llm(
        ...
        **kwargs: Any,
        ...
        :param query_cache_collection_name: The name of the collection to use for the query cache.
    ...
    def cache_query(
        ...
        """
        Cache one or more queries generated by the LLM only if they're not already stored.
        ...
        if queries is None:
            ...
            text = self._last_user_input.strip().lower() # Normalize the query text
            ...
            return "Cached: " + self._last_user_input
    ...
    def __get_cached_query(self, query_cache_similarity_threshold: float) -> Optional[Tuple[str, str]]:
        ...
        result = list(
            self.graph.db.aql.execute(  # type: ignore
                query, bind_vars={"embedding": embedding, "score_threshold": query_cache_similarity_threshold}
            )
        )
