...
class ArangoGraphQAChain(Chain):
    ...
    *Security note*: Make sure that the database connection uses credentials
    ...
    def cache_query(
        self, queries: Optional[List[Dict[str, str]]] = None
    ) -> Union[str, List[str]]:
        """
        Cache one or more queries generated by the LLM only if
        they're not already stored.

        If no list is provided, caches the most recent query.

        The query text is always lowercased before storing and checking for duplicates.

        :param queries: Optional list of dicts with 'text' and 'aql' keys.
        :return: True if a single new entry was inserted,
                or list of messages for multiple entries.
        """
        ...
        if queries is None:
            ...
            text = self._last_user_input.strip().lower()  # type: ignore
            aql = self._last_aql_query

            existing = list(collection.find({"text": text, "aql": aql}, limit=1))
            if existing:
                return "This query is already in the cache"
            ...
            return "Cached: " + self._last_user_input  # type: ignore
    ...
    def clear_query_cache(self, queries_to_remove: Optional[List[str]] = None) -> bool:
        """
        Clear the query cache.

        :param query_text: A list of queries to delete from the cache.
        ...
        if queries_to_remove is not None:
            ...
            remaining = list(collection.find({"text": {"$in": queries_to_remove}}))
    ...
    def __get_cached_query(
        self, query_cache_similarity_threshold: float
        ...
        query_text = self._last_user_input.strip().lower()  # type: ignore
        ...
        result = list(
            self.graph.db.aql.execute(  # type: ignore
                ...
                bind_vars={
                    "embedding": embedding,
                    "score_threshold": query_cache_similarity_threshold,
                },
            )
        )

        if result:
            return result[0]["aql"], f"{result[0]['score']:.2f}"
